...
int8_t mbedtls_hb2hex(uint8_t hb)
{
    hb = hb & 0xF;
...
void mbedtls_hmac_sha1( char *ref_hash,  
...
{  
    ...
    {  
        mbedtls_sha1((const unsigned char *)key, key_len, key_pad);  
    }  
    else  
    {  
...
void mbedtls_hmac_sha1_hex( char *ref_hash,  
    const void *key, size_t key_len,  
    const void *msg, size_t msg_len )  
{  
    mbedtls_sha1_context ctx;

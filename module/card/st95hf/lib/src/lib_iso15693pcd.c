...
{ 
    ...
    uint8_t	TagReply[RFTRANS_95HF_MAX_ECHO];
	uint8_t Tag_error_check;
	
    memset(TagReply , 0x00 , RFTRANS_95HF_MAX_ECHO);
	/* select 15693 protocol */
	errchk(ISO15693_Init( ));
...
Error:
    if( TagReply[1] > 16 )
    {
        log(WARN,"Get uuid return size is too long , size = %d\n" , TagReply[1]);
    }
	return ISO15693_ERRORCODE_DEFAULT;	
}

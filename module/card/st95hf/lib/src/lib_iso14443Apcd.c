...
static int8_t ISO14443A_AC( uint8_t *pDataRead, u8 CascadeLevel )
{
	...
	while( Collision == 0x80)
	{
		...
		{
			errCnt = 0;
			log_err(" ISO14443A_AC NFC read error \n");
		}
...
int8_t ISO14443A_PHONE_RATS( uint8_t *pDataRead )
{
	int8_t status;	
	uint8_t FSCI;
	uc8 	 pdata[]= { 0xE0, 0x50, 0x28};
...
int8_t ISO14443A_PPS( uint8_t *pDataRead )
{
...
int8_t ISO14443A_IsPresent( void )
{
    ...
    if(((*(pDataRead+2)==0x08)&&(*(pDataRead+3)==0x00)))
        ISO14443A_Card.cardType = BUS_CARD;
    else
        ISO14443A_Card.cardType=UNDEFINED_TAG_TYPE;

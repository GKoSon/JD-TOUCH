...
void esp12s_receive_byte( uint8_t ch )
{
    ...
    receiveTimecnt =0;
	if((( ch <= 'Z') && ( ch >= 'A')) || (( ch <= 'z') && ( ch >= 'a')) || (( ch <= '9') && ( ch >= '0')) || (ch == ' ') || (ch == '\r') || (ch == '\n') )
	{
		putchar(ch);
	}
...
static void wifi_rx_data_analisys(char *str , int len)
{
	...
	while(len-- > 0)
	{
        ...
        else if(aiot_strcmp(otp, "SEND OK" , strlen("SEND OK")) == true )
		{
			//int socket =0;
...
uint8_t wifi_rx_wait_cyc(char *cmd , int timeout)
{
	int cnt = timeout/10;
	uint8_t resault = FALSE;
	wifiReceiveMode = WIFI_COMMAND_MODE;
	
	while( (cnt--) && (resault == FALSE ))
	{
		if( receiveData.receiveFinsh == TRUE)
		{
			//log(DEBUG,"wifi receive: %.*s" , receiveData.len  , receiveData.rxBuff);
			if(strstr(receiveData.rxBuff , cmd) != NULL )
			{
				log(INFO,"\n接收到SEND OK\n");
				resault = TRUE;
			}
			wifi_rx_data_analisys(receiveData.rxBuff , receiveData.len);
...
{
    uint8_t comm[20] , cmdLength =0 ;
    
	//log(DEBUG,"esp12s 发送数据,id = %d ,length = %d , data = %s\n" , id ,length ,sendData);
    ...
    {    	
		...
		log(INFO,"wifi send data\n");
        wifi_send_data(sendData ,length );
		
		if( wifi_rx_wait_cyc("SEND OK" , 5000) == TRUE )
		{
...
void esp12s_close( void )
{
	...
	//else
	{
    	//log(DEBUG,"WIFI已经连接上AP ，暂时不重启WIFI模块\n");
	}

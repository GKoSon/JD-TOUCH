...
/*! FDT Poll adjustment for ISO14443A   EMV CCP Spec Book D v2.01  4.8.1.3  ;  Digital 1.1  6.10
 *
...
/*! FDT Poll adjustment for ISO14443B   EMV CCP Spec Book D v2.01  4.8.1.3  ;  Digital 1.1  7.9
 *
 ...
 */
#define RFAL_FDT_LISTEN_B_ADJUSTMENT    340
...
ReturnCode rfalCalibrate( void )
{
    ...
    /*******************************************************************************/
    /* Make use of the Calibration result */
    st25r3911ClrRegisterBits( ST25R3911_REG_ANT_CAL_CONTROL, ST25R3911_REG_ANT_CAL_CONTROL_trim_s );
...
static void rfalErrorHandling( void )
{
    ...
    /*******************************************************************************/
    /* Read FIFO Status */
    //st25r3911ReadRegister(ST25R3911_REG_FIFO_RX_STATUS1, &fifoBytesToRead);
    //st25r3911ReadRegister(ST25R3911_REG_FIFO_RX_STATUS2, &fifoStatus);
    ...
    {
        /*******************************************************************************/
        /* EMD Handling - NFC Forum Digital 1.1 4 ; EMVCo                              */
        /* ReEnable the receiver upon:                                                 */
        /*   - Collision or Framing error detected                                     */
        /*   - Residual bits are detected (hard framing error)                         */
        /*   - Parity error when the received msg is not bigger then 4 bytes           */
        /*   - CRC error when the received msg is not > 4 bytes, nor < 2 bytes         */
        /*******************************************************************************/
        if(  (gRFAL.TxRx.status == ERR_RF_COLLISION) || (gRFAL.TxRx.status == ERR_FRAMING)                          ||
             (fifoStatus & (ST25R3911_REG_FIFO_RX_STATUS2_fifo_ncp | ST25R3911_REG_FIFO_RX_STATUS2_np_lb))                ||
            ((gRFAL.TxRx.status == ERR_PAR) && ((gRFAL.fifo.bytesTotal + fifoBytesToRead) < RFAL_EMVCO_RX_MAXLEN )) ||
            ((gRFAL.TxRx.status == ERR_CRC) && ((gRFAL.fifo.bytesTotal + fifoBytesToRead) < RFAL_EMVCO_RX_MAXLEN )  && ((gRFAL.fifo.bytesTotal + fifoBytesToRead) >= RFAL_EMVCO_RX_MINLEN )))
        {
...
static void rfalTransceiveRx( void )
{
    ...
    switch( gRFAL.TxRx.state )
    {
        ...
        case RFAL_TXRX_STATE_RX_READ_FIFO:
            ...
            aux = (( gRFAL.fifo.bytesTotal > gRFAL.TxRx.ctx.rxBufLen ) ? (gRFAL.TxRx.ctx.rxBufLen - gRFAL.fifo.bytesWritten) : tmp);

...
ReturnCode rfalNfcbPollerCheckPresence( rfalNfcbSensCmd cmd, rfalNfcbSlots slots, rfalNfcbSensbRes *sensbRes, uint8_t *sensbResLen )
{
    ...
    *sensbResLen = 0;
    ...
    ret = rfalTransceiveBlockingTxRx( (uint8_t*)&sensbReq, sizeof(rfalNfcbSensbReq), (uint8_t*)sensbRes, sizeof(rfalNfcbSensbRes), &rxLen, RFAL_TXRX_FLAGS_DEFAULT, RFAL_NFCB_ACTIVATION_FWT );
    *sensbResLen = rxLen;
...
ReturnCode rfalNfcbPollerCollisionResolution( rfalComplianceMode compMode, uint8_t devLimit, rfalNfcbListenDevice *nfcbDevList, uint8_t *devCnt )
{
    ...
    /* Check parameters */
    if( (nfcbDevList == NULL) || (devCnt == NULL) )
    {
        return ERR_PARAM;
    }
    ...
    *devCnt    = 0;
    ...
    if( compMode == RFAL_COMPLIANCE_MODE_NFC_11 )
    {
       EXIT_ON_ERR( ret, rfalNfcbPollerCheckPresence( RFAL_NFCB_SENS_CMD_ALLB_REQ, RFAL_NFCB_SLOT_NUM_1, &nfcbDevList->sensbRes, &nfcbDevList->sensbResLen ));
    }
    
    
    for( slotsNum = RFAL_NFCB_SLOT_NUM_1; slotsNum <= RFAL_NFCB_SLOT_NUM_16; slotsNum++ )
    {
        /* Activity 1.1  9.3.5.23  -  Symbol 22 */
        if( (compMode == RFAL_COMPLIANCE_MODE_NFC_11) && (slotsNum != RFAL_NFCB_SLOT_NUM_1) && (*devCnt != 0) )
        {
            rfalNfcbPollerSleep( nfcbDevList[(*devCnt-1)].sensbRes.nfcid0 );
            nfcbDevList[(*devCnt-1)].isSleep = true;
        }
        
        /* Send SENSB_REQ with number of slots if not the first Activity 1.1  9.3.5.24  -  Symbol 23 */
        if( slotsNum != RFAL_NFCB_SLOT_NUM_1 )
        {
           ret = rfalNfcbPollerCheckPresence( RFAL_NFCB_SENS_CMD_SENSB_REQ, slotsNum, &nfcbDevList[*devCnt].sensbRes, &nfcbDevList[*devCnt].sensbResLen );
        }
        ...
        do{
            ...
            if( ret != ERR_TIMEOUT )
            {
                ...
                if( (ret == ERR_NONE) && (rfalNfcbCheckSensbRes( &nfcbDevList[*devCnt].sensbRes, nfcbDevList[*devCnt].sensbResLen) == ERR_NONE) )
                {
                    ...
                    if( compMode == RFAL_COMPLIANCE_MODE_NFC_10 )
                    {
                    ...
                    else if( compMode == RFAL_COMPLIANCE_MODE_NFC_11 )
                    {

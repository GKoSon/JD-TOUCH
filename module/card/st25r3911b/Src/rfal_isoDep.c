...
#define ISODEP_PCB_IB_VALID_MASK        (ISODEP_PCB_B2_BIT)                                         /*!< Bit mask for the MUST bits on I-Block    */
#define ISODEP_PCB_IB_VALID_VAL         (ISODEP_PCB_B2_BIT)                                         /*!< Value for the MUST bits on I-Block       */
#define ISODEP_PCB_RB_VALID_MASK        (ISODEP_PCB_B6_BIT | ISODEP_PCB_B3_BIT)                     /*!< Bit mask for the MUST bits on R-Block    */
#define ISODEP_PCB_RB_VALID_VAL         (ISODEP_PCB_B6_BIT)                                         /*!< Value for the MUST bits on R-Block       */
...
#define ISODEP_MAX_I_RETRYS             (3)         /*!< Number of retrys for a I-Block                                                    */
#define ISODEP_MAX_R_RETRYS             (3)         /*!< Number of retrys for a R-Block, Digital 1.0 A9 - nRETRY ACK/NAK:  [2,5]           */
#define ISODEP_MAX_S_RETRYS             (3)         /*!< Number of retrys for a S-Block, Digital 1.0 A9 - nRETRY DESELECT: [0,5] WTX[2,5]  */
...
#define ISODEP_FWT_LIS_MAX              rfalIsoDepFWI2FWT(8)        /*!< FWT Listener Max  FWIT4ATmax FWIBmax  Digital 1.1  A6 & A3 */
...
/*! Delta Time for polling during Activation (ATS) : 16.4ms  Digital 1.1 13.8.1.1 & A.9    */
#define RFAL_ISODEP_T4T_DTIME_POLL_11          rfalConvMsTo1fc(17)

/*! Activation frame waiting time FWT(act) = 71680/fc (~5286us) Digital 1.1 13.8.1.1 & A.9 */
#define RFAL_ISODEP_T4T_FWT_ACTIVATION         (71680 + RFAL_ISODEP_T4T_DTIME_POLL_10)


/*! Delta frame waiting time = 16/fc  Digital 1.0  11.7.1.3 & A.7*/
#define RFAL_ISODEP_DFWT_10                      16

/*! Delta frame waiting time = 16/fc  Digital 1.1  13.8.1.3 & A.7*/
...
#define isoDep_PCBIBlockChaining( bn )  ( (uint8_t)(isoDep_PCBIBlock(bn) | ISODEP_PCB_CHAINING_BIT))                              /*!< Returns an I-Block with the given block number (bn) indicating chaing */
...
static ReturnCode isoDepHandleControlMsg( rfalIsoDepControlMsg controlMsg, uint8_t param )
{
    ...
    switch( controlMsg )
    {
        ...
        case ISODEP_R_ACK:
            ...
            if( gIsoDep.cntRRetrys++ > ISODEP_MAX_R_RETRYS )
            {
                return ERR_PROTO;
            }
        ...
        case ISODEP_ST_PCD_RX:
            ...
            switch( ret )
            {
                ...
                case ERR_INCOMPLETE_BYTE:  /* added to handle test cases scenario TC_POL_NFCB_T4AT_BI_82_x_y & TC_POL_NFCB_T4BT_BI_82_x_y  */
                    ...
                    {   /* Rule 4 - When a invalid block or timeout occurs -> R-ACK */
            ...
            else if( isoDep_PCBisRBlock(rxPCB) )
            {
                if( isoDep_PCBisRACK(rxPCB) )                             /* Check if is a R-ACK */
                {
                    ...
                    else
                    {
                        ...
                        if( gIsoDep.cntIRetrys++ < ISODEP_MAX_I_RETRYS )
                        {                        
...
uint32_t rfalIsoDepFWI2FWT( uint8_t fwi )
{
    ...
    if (fwi > ISODEP_FWI_MAX || (fwi < ISODEP_FWI_MIN_10) )
    {   
...
#define isoDepCalcdSGFT( s )      (384  * (1 << s))   /*!< Calculates the dSFGT with given SFGI  Digital 1.1  13.8.2.1 & A.6*/
#define isoDepCalcSGFT( s )       (4096 * (1 << s))   /*!< Calculates the SFGT with given SFGI  Digital 1.1  13.8.2         */
...
ReturnCode rfalIsoDepListenStartActivation( rfalIsoDepAtsParam *atsParam, rfalIsoDepAttribResParam *attribResParam, uint8_t *buf, uint16_t bufLen, rfalIsoDepListenActvParam actParam)
{
    ...
    return rfalTransceiveBlockingTx( txBuf, bufIt, (uint8_t*)actParam.rxBuf, sizeof( rfalIsoDepBufFormat ), actParam.rxBufLen, RFAL_TXRX_FLAGS_DEFAULT, RFAL_FWT_NONE );
}
...
ReturnCode rfalIsoDepListenGetActivationStatus( void )
{
    ...
    switch( err )
    {
        ...
        default:
            ...
            isoDepReEnableRx( (uint8_t*)gIsoDep.actvParam.rxBuf, sizeof( rfalIsoDepBufFormat ), gIsoDep.actvParam.rxBufLen );
    ...
    if( gIsoDep.state == ISODEP_ST_PICC_ACT_ATS )
    {
        ...
        if( ( ((uint8_t*)gIsoDep.actvParam.rxBuf)[RFAL_ISODEP_PPS_STARTBYTE_POS] & RFAL_ISODEP_PPS_MASK) == RFAL_ISODEP_PPS_SB )
        {
            ...
            rfalTransceiveBlockingTx( txBuf, bufIt, (uint8_t*)gIsoDep.actvParam.rxBuf, sizeof( rfalIsoDepBufFormat ), gIsoDep.actvParam.rxBufLen, RFAL_TXRX_FLAGS_DEFAULT, RFAL_FWT_NONE );
    ...
    gIsoDep.rxLen       = gIsoDep.actvParam.rxBufLen;
...
ReturnCode rfalIsoDepRATS( rfalIsoDepFSxI FSDI, uint8_t DID, rfalIsoDepAts *ats , uint8_t *atsLen)
{
    ...
    ratsReq.PARAM = ((fsdiVal << RFAL_ISODEP_RATS_PARAM_FSDI_SHIFT) & RFAL_ISODEP_RATS_PARAM_FSDI_MASK) | (DID & RFAL_ISODEP_RATS_PARAM_DID_MASK);
...
/*******************************************************************************/
ReturnCode rfalIsoDepPollAHandleActivation( rfalComplianceMode compMode, rfalIsoDepFSxI FSDI, uint8_t DID, rfalBitRate maxBR, rfalIsoDepDevice *isoDepDev )
{
    uint8_t          RATSretries;
    uint8_t          msgIt;
    ReturnCode       ret;
    ...
    RATSretries = RFAL_ISODEP_RATS_RETRIES;
...
ReturnCode rfalIsoDepPollBHandleActivation( rfalIsoDepFSxI FSDI, uint8_t DID, rfalBitRate maxBR, uint8_t PARAM1, rfalNfcbListenDevice *nfcbDev, uint8_t* HLInfo, uint8_t HLInfoLen, rfalIsoDepDevice *isoDepDev )
{
    ...
    ret = rfalIsoDepATTRIB( (uint8_t*)&nfcbDev->sensbRes.nfcid0,
                           ...
                           isoDepDev->info.DRI,
                           FSDI,
                           (nfcbDev->sensbRes.protInfo.FsciProType & ( (RFAL_NFCB_SENSB_RES_PROTO_TR2_MASK<<RFAL_NFCB_SENSB_RES_PROTO_TR2_SHIFT) | RFAL_NFCB_SENSB_RES_PROTO_ISO_MASK)),
                           DID,
                           HLInfo,
                           HLInfoLen,
 ...
 static void rfalIsoDepApdu2IBLockParam( rfalIsoDepApduTxRxParam apduParam, rfalIsoDepTxRxParam *iBlockParam, uint16_t txPos, uint16_t rxPos )
{
     ...
     iBlockParam->isRxChaining = &gIsoDep.isRxChaining;
     iBlockParam->rxBufLen     = apduParam.rxBufLen;
}
...
ReturnCode rfalIsoDepGetApduTransceiveStatus( void )
{
    ...
    switch( ret )
    {
        ...
        case ERR_NONE:
            ...
            ST_MEMCPY( &gIsoDep.APDUParam.rxBuf->apdu[gIsoDep.APDURxPos], gIsoDep.APDUParam.tmpBuf->inf, *gIsoDep.APDUParam.rxBufLen );
            gIsoDep.APDURxPos += *gIsoDep.APDUParam.rxBufLen;
    ...
    *gIsoDep.APDUParam.rxBufLen = gIsoDep.APDURxPos;
